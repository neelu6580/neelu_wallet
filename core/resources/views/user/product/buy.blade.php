                ...
                <div class="card card-profile bg-white border-0 mb-5">
                    <div class="row justify-content-center">
                        <div class="col-lg-3 order-lg-2">
                            ...
                            <img  @if(empty($logo_product_img))src="{{url('/')}}/asset/profile/{{$merchant->image}}" @else src="{{url('/')}}/asset/{{$logo_product_img}}" @endif class="rounded-circle border-secondary">
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="{{route('pay.product')}}" method="post" id="modal-details">
                            @csrf
                            <div class="row justify-content-between align-items-center">
                                <div class="col">
                                    @if($product->quantity_status==0)
                                        @if($product->quantity!=0)
                                            <div class="col-lg-4">
                                                <span class="badge badge-pill badge-primary mb-3">{{__('In stock')}}: {{$product->quantity}}</span>
                                            </div>
                                        @endif
                                    @endif
                                </div>
                                <div class="col-auto">
                                    <div class="text-right">                        
                                        @if($product->quantity_status==1)
                                            @if($product->status==1)
                                                <button type="button" data-toggle="modal" data-target="#fund" class="btn btn-success my-4 btn-sm">{{__('Pay with Card')}}</button>
                                            @else
                                                <button type="submit" disabled class="btn btn-success btn-sm">{{__('NOT AVAILABLE')}}</button>
                                            @endif                                                             
                            ...
                            <span class="form-text text-xs text-default">Product Details: {!!$product->description!!}.</span>
                            @endif
                            @if($product->quantity_status==0)
                            ...
                            @endif
                                ...
                                <?php if(session()->has('new_to')){ ?>
                                    <input type="hidden" value="{{ bcdiv($lowest_rate, 1, 2) }}" name="shipping_fee">
                                <?php } ?>
                                ...
                                <?php 
                                    if(session()->has('new_to')){ 
                                       ...
                                       // dd($shipment->id);
                                ?>
                                    <input type="hidden" value="{{ $shipment->id }}" name="buy_shipment">
                                <?php }else{ ?>
                            ...
                            <div class="form-group row">
                                <div class="col-lg-12">
                                    <textarea type="text" name="note" class="form-control" placeholder="Delivery Note @if($product->note_status==1)(Optional) @endif" @if($product->note_status==2)required="" @endif></textarea>
                                </div>
                            </div>
                            @endif
                            @if($product->shipping_status==1)
                                ...
                                <div class="modal-dialog modal- modal-dialog-centered modal-md" role="document">
                                    ...
                                    <div class="modal-body p-0">
                                        ...
                                        <div class="card bg-white border-0 mb-0">
                                            ...
                                            <div class="card-body">
                                                <form action="{{ route('pay.product')}}" role="form" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{$stripe->val1}}" id="payment-form">
                                                @csrf
                                                <div class="form-group row">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row justify-content-between align-items-center">
                            ...
                            <div class="col-auto">
                                @if(!empty($all_rates))
                                    @if($all_rates == 'No Rates Found')
                                        <tr>
                                            <td>
                                                <div class="radio r1">
                                                    <span class="text-sm text-default">{{__('Flat rate')}}: {{$currency->symbol}}0.00</span>
                                                </div>
                                            </td>
                                        </tr>
                                    @else
                                        @foreach($all_rates as $rate)
                                            ...
                                            ?>
                                                <span class="text-sm text-default">{{__('Flat rate')}}: {{$currency->symbol}}{{ bcdiv($total_rate, 1, 2) }}</span>
                                            <?php } ?>
                                        @endforeach
                                    @endif
                                @else
                                    <span class="text-sm text-default">{{__('Flat rate')}}: {{$currency->symbol}}0.00</span>
                                @endif
                                <!--<span class="text-sm text-default">{{__('Flat rate')}}: {{$currency->symbol}}{{$ship_fee}}.00</span>-->
                            </div>
                        </div>
                        <hr>
                        @endif
                                ...
                                @if(!empty($all_rates))
                                    @if($all_rates == 'No Rates Found')
                                        <tr>
                                            <td>
                                                <div class="radio r1">
                                                   <span class="text-sm text-default">{{$currency->symbol}}<span id="total1">{{$subtotal}}</span>.00</span>
                                                </div>
                                            </td>
                                        </tr>
                                    @else
                                        @foreach($all_rates as $rate)
                                            <?php 
                                                ...
                                                if ($lowest_rate == $rate['rate']){
                                            ?>
                                                <span class="text-sm text-default">{{__('Flat rate')}}: {{$currency->symbol}}{{ bcdiv($grand_total, 1, 2) }}</span>
                                            <?php } ?>
                                        @endforeach
                                    @endif
                                @else
                                    <span class="text-sm text-default">{{$currency->symbol}}{{$subtotal}}.00</span>
                                @endif
                                        ...
                                        <div class="carousel-inner bg-transparent">
                                            @if($product->new==0)
                                                <div class="carousel-item active">
                                                    <img class="d-block w-80" @if(empty($logo_product_img))src="{{url('/')}}/asset/profile/{{$logo->image_link2}}" @else src="{{url('/')}}/asset/{{$logo_product_img}}" @endif  alt="product image">
                                                </div>
                                            @else
                                                @foreach($image as $k=>$val)
                                                <div class="carousel-item bg-transparent @if($val->id==$first->id)active @endif">
                                                    <img class="d-block w-100" @if(empty($logo_product_img))src="{{url('/')}}/asset/profile/{{$val->image}}" @else src="{{url('/')}}/asset/{{$logo_product_img}}" @endif  alt="product image">
                                                </div>
                                                @endforeach
                                            @endif 
                                        </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form method="get" action="{{ url('buy-product/'.$product->ref_id)}}">
                @csrf
